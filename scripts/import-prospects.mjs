...
const chanOnly = fresh.filter((p) => yearOf(p) >= 2025).length;
const shared = fresh.filter((p) => yearOf(p) <= 2024).length;
...
const custRows = fresh.map((p) => ({ name: p.name, phone: p.phone, customer_type: custType(p.name), owner_id: CH, is_prospect: true, is_shared: yearOf(p) <= 2024 }));
const custIds = [];
